...
void l1r_l2_svc::solve_l1r_l2_svc(float64_t *w, float64_t eps, float64_t Cp, float64_t Cn)
{
	...
	for(j=0; j<w_size; j++)
	{
		...
		{
			int32_t ind = k;
			float64_t val = x_matrix[k][j];
			x_matrix[k][j] *= prob->y[ind]; // x->value stores yi*xij
			...
			for(num_linesearch=0; num_linesearch < max_num_linesearch; num_linesearch++)
			{
				...
				if(num_linesearch == 0)
				{
					...
					for(k=0; k<l; k++)
					{
						...
						float64_t b_new = b[ind] + d_diff*x_matrix[k][j];
						b[ind] = b_new;
						if(b_new > 0)
	...
	for(j=0; j<w_size; j++)
	{
		for(k=0; k<l; k++)
		{
			x_matrix[k][j] *= prob->y[k]; // restore x->value
		}
		if(w[j] != 0)
		{
			v += fabs(w[j]);
