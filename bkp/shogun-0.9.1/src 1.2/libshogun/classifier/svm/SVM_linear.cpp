...
void l1r_l2_svc::solve_l1r_l2_svc(float64_t *w, float64_t eps, float64_t Cp, float64_t Cn)
{
	...
	//float64_t x_matrix[l][w_size];
	//prob->x->get_feature_matrix(&x_matrix, num_vectors, num_features);
	...
	printf("mmatx=%f %f %f %f %f %f %f %f \n", x_matrix[0], x_matrix[1], x_matrix[2], x_matrix[3], x_matrix[4], x_matrix[5], x_matrix[6], x_matrix[7]);
	//printf(" %s", typeof(*x_matrix));
	...
	for(j=0; j<num_feat; j++)
	{
		...
		for(k=0; k<l; k++)
		{	
			printf("j=%d, k=%d , val=%f - ", j,k, *(x_matrix + k*num_feat + j));
